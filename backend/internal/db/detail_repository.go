...
func (r *DetailRepository) CreateDetail(detail *Detail) error {
	...
	logger.Info("成功创建详单记录 - 房间ID: %d, 开始时间: %v, 费用: %.2f, 风速: %s",
		detail.RoomID, detail.StartTime.Format("15:04:05"), detail.Cost, detail.Speed)
	return nil
}
...
func (r *DetailRepository) GetTotalCost(roomID int, startTime, endTime time.Time) (float32, error) {
	...
	logger.Info("成功计算总费用 - 房间ID: %d, 总费用: %.2f元", roomID, totalCost)
